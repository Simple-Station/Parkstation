...
public sealed class DrinkSystem : EntitySystem
{
    ...
    private bool TryDrink(EntityUid user, EntityUid target, DrinkComponent drink, EntityUid item)
    {
        ...
        else
        {
            ...
            flavors = HasComp<AgeusiaComponent>(user) == true ? "Feels like water." : _flavorProfile.GetLocalizedFlavorsMessage(user, drinkSolution);

            var voraciousDelay = drink.Delay;
            if (_entity.TryGetComponent<VoraciousTraitComponent>(target, out var voracious) &&
