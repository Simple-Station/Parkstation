...
namespace Content.Server.Nutrition.EntitySystems
{
    ...
    public sealed class DrinkSystem : EntitySystem
    {
        ...
        private bool TryDrink(EntityUid user, EntityUid target, DrinkComponent drink, EntityUid item)
        {
            ...
            flavors = HasComp<AgeusiaComponent>(user) == true ? "Feels like water." : _flavorProfileSystem.GetLocalizedFlavorsMessage(user, drinkSolution);

            var voraciousDelay = drink.Delay;
            if (_entityManager.TryGetComponent<VoraciousTraitComponent>(target, out var voracious) &&
                voracious.NegateHunger != null)
                voraciousDelay -= (float) voracious.NegateHunger;

            var doAfterEventArgs = new DoAfterArgs(
                user,
                forceDrink ? drink.ForceFeedDelay : voraciousDelay,
